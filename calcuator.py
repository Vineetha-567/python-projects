...
root.title("VinnySamy calculator")
...
def equal():
    ...
    if math == "add":
        ...
        e.insert(0,fnum**int(secondnum))
# functions that calcuate globle is used to access another funtions
def sub():
    firstnum=e.get()
    global fnum 
    global math 
...
buttonmu1=Button(root,text="*",padx=40,pady=20,command=mu1)
...
buttonmu1.grid(row=5,column=0)
buttondiv.grid(row=5,column=1)
buttonclear.grid(row=5,column=2)
buttonequal.grid(row=6,column=0)
